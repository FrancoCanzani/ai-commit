...
export async function setup() {
    ...
    const envVariable = await input({
        ...
        default: provider === AI_PROVIDERS.OPENAI ? 'OPENAI_API_KEY' : 'ANTHROPIC_API_KEY',
    });
    ...
    const language = await select({
        ...
        choices: Object.keys(COMMIT_LANGUAGES).map((lang) => ({
            name: lang,
    ...
    await fs.writeFile(path.join(process.cwd(), 'ai-commit-rc.json'), JSON.stringify(config, null, 2));
