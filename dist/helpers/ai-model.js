...
export default class AiModel {
    ...
    async generateCommitMessage(diff) {
        ...
        try {
            ...
            if (!modelType) {
                throw new Error(`Model type for provider ${this.provider} is not supported or incorrectly configured.`);
            }
            const result = await streamText({
                model: modelType,
                prompt: `Based on this git diff, write a commit message in ${config?.language ?? 'english'}. ${config?.format === COMMIT_FORMATS.SIMPLE
                    ? 'Write a simple description without any type or scope prefixes.'
                    : `Use the format ${config?.format}`}. Maximum ${config?.maxLength ?? 50} characters. No explanation, just the commit message. ${diff}`,
                headers: {
            ...
            throw new Error('Failed to generate commit message. Please check your configuration and API credentials.');
        }
    }
}
